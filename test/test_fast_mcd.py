...
def test_gaussian(n, p, means, vars):
    """
    ...
    if PLOT:
        plt.plot(x, template.pdf(x), linestyle='--', color='blue')
    mse_naive = ((fh - template.pdf(x))**2).mean()
    imse_naive = 0.5 * ((fh - template.pdf(x))**2).sum() * (x[1] - x[0])
    if PLOT:
    ...
    mse_robust = ((fh - template.pdf(x))**2).mean()
    imse_robust = 0.5 * ((fh - template.pdf(x))**2).sum() * (x[1] - x[0])
    if PLOT:
...
if __name__ == "__main__":
    p_max = 10
    nb_loops = 10
    accu_mse_naive = np.zeros(p_max-1)
    accu_mse_robust = np.zeros(p_max-1)
    accu_imse_naive = np.zeros(p_max-1)
    accu_imse_robust = np.zeros(p_max-1)
    for p in range(2,p_max+1):
        print p
        for i in range(nb_loops):
            mse_naive, mse_robust, imse_naive, imse_robust = test_gaussian(
                700, p, np.tile([0], p), np.tile([1], p))
            accu_mse_naive[p-2] += mse_naive
            accu_mse_robust[p-2] += mse_robust
            accu_imse_naive[p-2] += imse_naive
            accu_imse_robust[p-2] += imse_robust
